...
" marks
hi SignColumn ctermfg=black ctermbg=white
hi SignatureMarkText ctermfg=black ctermbg=White
...
" Function to add HTML anchors to Markdown headings within a specified range
" Args:
"     start (int): The starting line number of the range.
"     end (int): The ending line number of the range.
function! AddAnchorsToMarkdownHeadingsInRange(start, end) abort
    " Execute a substitution command on lines within the given range
    " This command finds Markdown headings and appends an HTML anchor tag
    " with an ID generated by the Slugify function.
    execute a:start . ',' . a:end . 'g/^\s*#\+\s\+.*$/s/^\(\s*#\+\)\s\+\(.*\)$/\=submatch(1) . " " . submatch(2) . " <a id=\"#" . Slugify(submatch(2)) . "\/>"/'
    " Notify user that anchors have been added
    echo "Markdown anchors added to headings in range"
endfunction

" Key mapping in visual mode to call the function on a selected range
" Pressing <leader>mm will trigger the function for the selected text.
